...
variable "subnet_ids" {
  ...
  description = <<-EOT
    ...
    An entry may be left empty (""), so a placeholder for a subnet you have not
    created yet can sit in this file without breaking the plan. Only entries an
    instance actually points at are read, so an empty one costs nothing - but an
    instance whose subnet_key resolves to "" fails at plan time.
  EOT
  default     = {}
