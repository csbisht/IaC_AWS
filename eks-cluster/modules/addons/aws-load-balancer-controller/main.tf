...
resource "aws_iam_policy" "lb_controller_policy" {
  name        = "LoadBalancerControllerIAMPolicy-${var.cluster_name}"
...
###############################
# IRSA role for the Load Balancer Controller
###############################
resource "aws_iam_role" "lb_controller_role" {
  name = "LoadBalancerControllerRole-${var.cluster_name}"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringEquals = {
            ...
            "${var.oidc_provider_url}:sub" = "system:serviceaccount:${var.lb_namespace}:${var.lb_sa_name}"
          }
        }
      }
    ]
...
resource "helm_release" "aws_load_balancer_controller" {
  ...
  namespace  = var.lb_namespace
  repository = var.lb-controller_chart_repository
  chart      = "aws-load-balancer-controller"
  version    = var.lb_chart_version
  ...
  set = [
    ...
    },
    {
      ...
      value = "false" # Do not create a new service account
    },
    {
      name  = "serviceAccount.name"
      value = var.lb_sa_name # Use the custom service account
    },
    {
      name  = "vpcId"
      value = var.vpc_id
  ...
  depends_on = [
    ...
    kubernetes_service_account.custom_sa
  ]
}
