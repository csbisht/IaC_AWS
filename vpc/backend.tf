...
#
# A VPC is usually the longest-lived state in the account and the one every
# other configuration reads ids out of, so S3 plus a lock table is worth setting
# up here before it is worth setting up anywhere else.
#
terraform {
  backend "local" {
