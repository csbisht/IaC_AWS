# Root variable interface. main.tf forwards these into modules/vpc one for one,
...
variable "aws_region" {
  type        = string
  description = "AWS region the VPC is created in. Consumed by the provider in providers.tf and by the endpoint service names in the module."
}

variable "name_prefix" {
  type        = string
  description = "Base name for every resource. The VPC becomes \"<name_prefix>-vpc\", a subnet \"<name_prefix>-<group>-<az suffix>\" (e.g. \"tf-example-pvt-1a\"), the gateways \"<name_prefix>-igw\" and \"<name_prefix>-nat-1a\"."
}
...
variable "vpc_cidr" {
  type        = string
  description = <<-EOT
    IPv4 CIDR of the VPC, between /16 and /28. It cannot be changed later
    without replacing the VPC, so leave room: every subnet below has to fit
    inside it, and an EKS cluster spends one VPC address per pod.
  EOT
...
# Subnets
#
# The whole layout is data: one entry per tier, each holding a map of
# availability zone -> CIDR. Adding an AZ is a line, adding a tier is a block,
# and neither touches what already exists - the map keys are the for_each
# identity, so nothing is renumbered the way a list would be.
########################################
...
variable "nat_gateway_mode" {
  type        = string
  description = <<-EOT
    How many NAT gateways to create for the private subnets. This is the line
    item that dominates the bill of an idle VPC - a gateway is charged per hour
    whether or not anything is behind it.
      ...
      one_per_az - one NAT gateway per AZ that has a public subnet, with each
                   private route table pointing at the one in its own AZ. The
                   production layout, at a multiple of the cost.
  EOT
  default     = "single"
